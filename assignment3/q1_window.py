...
def make_windowed_data(data, start, end, window_size=1):
    ...
    for sentence, labels in data:
        ...
        sentence_pad = ([start] * window_size) + sentence + ([end] * window_size)
        window_len = (2 * window_size) + 1
        for i in range(0, len(sentence)):
            x = []
            for features in sentence_pad[i:i + window_len]:
                x += features
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        input_embeddings = tf.nn.embedding_lookup(all_embeddings, self.input_placeholder)
        embeddings = tf.reshape(input_embeddings, shape=(-1, self.config.n_window_features * self.config.embed_size))
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        # h = Relu(xW + b1)
        W = tf.get_variable('W', initializer=tf.contrib.layers.xavier_initializer(), shape=(n_window_features * embedding_size, hidden_size))
        b1 = tf.get_variable('b1', initializer=tf.contrib.layers.xavier_initializer(), shape=(hidden_size,))
        ...
        # pred = h_dropU + b2
        U = tf.get_variable('U', initializer=tf.contrib.layers.xavier_initializer(), shape=(hidden_size, n_classes))
        b2 = tf.get_variable('b2', initializer=tf.contrib.layers.xavier_initializer(), shape=(n_classes,))
