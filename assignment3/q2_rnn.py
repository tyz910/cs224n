...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        sen_len = min(len(sentence), max_length)
        pad_len = max_length - sen_len

        sentence_pad = sentence[:sen_len] + [zero_vector] * pad_len
        labels_pad = labels[:sen_len] + [zero_label] * pad_len
        mask = [True] * sen_len + [False] * pad_len
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(input_embeddings, shape=(-1, self.max_length, self.config.n_features * self.config.embed_size))
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        b2 = tf.get_variable('b2', initializer=tf.contrib.layers.xavier_initializer(), shape=(1, self.config.n_classes))
